...
from tushare.util import dateu as du, conns
    ...
    for _ in range(retry_count):
        try:
            ...
            if ktype in ct.KTYPE_LOW_COLS:
                data = pd.DataFrame()
                for i in range(100): 
                    ds = func(ct.KTYPE[ktype], mkcode, code, i * 800, row_count)
                    df =  api.to_df(ds)
...
if __name__ == '__main__':
    cons = conns.get_apis()
    print(cons)
    pd = bar(code='000001', conn=cons, start_date='2017-01-01', end_date='2017-10-31', freq='1MIN', asset='E', 
               market='',
               adj = None,
               ma = [],
               factors = [],
